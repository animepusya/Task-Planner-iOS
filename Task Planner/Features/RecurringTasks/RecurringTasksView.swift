...
struct RecurringTasksView: View {
                ...
                List {
                    if viewModel.isLoading && sections.active.isEmpty && sections.past.isEmpty {
                        ProgressView()
                            ...
                            .padding(.vertical, metrics.spacing(DS.Spacing.xl))
                            .dsContentFrame(.modal)
                            .listRowInsets(.init(top: metrics.spacing(DS.Spacing.md), leading: metrics.screenPadding(DS.Spacing.lg), bottom: metrics.spacing(28), trailing: metrics.screenPadding(DS.Spacing.lg)))
                        ...
                        if !sections.active.isEmpty {
                            Section {
                                ForEach(sections.active) { task in
                                    recurringTaskRow(task, metrics: metrics)
                                }
                            } header: {
    ...
    private func recurringTaskRow(
        ...
        RecurringTaskCardView(task: task)
            ...
            .dsContentFrame(.modal)
            .listRowInsets(.init(top: 0, leading: metrics.screenPadding(DS.Spacing.lg), bottom: 0, trailing: metrics.screenPadding(DS.Spacing.lg)))
            .listRowBackground(Color.clear)
            .listRowSeparator(.hidden)
    }
}
